...
pt_direct_constellation =  qammod([0 pt_M-1], pt_M, 0);
pt_direct_average_symbol_energy = mean(abs(pt_direct_constellation));

%Relay Phase
pt_bits = 6;
pt_M = 2.^pt_bits;
pt_relay_constellation = qammod([0 pt_M-1], pt_M, 0);
pt_relay_average_symbol_energy = mean(abs(pt_relay_constellation));
...
for m = 1:length(snr_dB)
	...
	pt_direct_pr_sigma = sqrt(pt_direct_average_symbol_energy/(2.*pt_direct_pr_snr));
	
	pt_relay_pr_snr = snr_dB(m) - (n.*10.*log10(d_pt_pr));
	pt_relay_pr_snr = 10.^(pt_relay_pr_snr/10);
	pt_relay_pr_sigma = sqrt(pt_relay_average_symbol_energy/(2.*pt_relay_pr_snr));
	
	pt_st_snr = snr_dB(m) - (n.*10.*log10(d_pt_st));
	pt_st_snr = 10.^(pt_st_snr/10);
	pt_st_sigma = sqrt(pt_relay_average_symbol_energy/(2.*pt_st_snr));
	...
	while errors_pu_direct < 200
		...
		%At primary reciever
		pt_direct_pr_h = (randn(1,1)+i*randn(1,1))./sqrt(2);

		pt_direct_pr_n = pt_direct_pr_sigma*((randn(1,1)+i*randn(1,1))./sqrt(2));
		...
		%At secondary transmitter
			...
			st_pr_sigma = sqrt((norm(st_stbc_code,'fro')^2)/(4*(10.^(snr_dB(m)/10))));
			st_sr_sigma = sqrt((norm(st_stbc_code,'fro')^2)/(4*(10.^(snr_dB(m)/10))));
		%At primary receiver
