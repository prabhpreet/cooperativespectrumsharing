...
for m = 1:length(snr)
	...
	while errors_pu_direct < 200
		...
		%%%%%%%%
			...
			pt_direct_h = d_pt_pr.*randn(1, 1) + j.*randn(1,1)./sqrt(2);
			
			pt_direct_n = sqrt((norm(pt_direct_x(1),'fro')^2)/(snr(m)))*((randn(1,1)+j*randn(1,1))./sqrt(2));
		...
		%%%%%%%%
			...
			%Assuming ST is very close to PT and perfect decoding
			pt_symbols = randi([1,4], 1,3);

			pt_bit_sequence = reshape(de2bi(pt_symbols-1,2)', 1, 6);
